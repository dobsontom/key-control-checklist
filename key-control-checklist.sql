WITH
  date_scaffold AS (
    ...
    FROM
      UNNEST (
        GENERATE_DATE_ARRAY(DATE_ADD(CURRENT_DATE, INTERVAL - 5 DAY), DATE_ADD(CURRENT_DATE, INTERVAL - 1 DAY))
      ) AS date
  ),
  breakdown_scaffold AS (
   SELECT DISTINCT
      ErrorMessageID AS breakdown
    FROM revenue-assurance-prod.control_f12m_btp_suspense.tableau_summary
   UNION
   SELECT DISTINCT
      ErrorMessageID AS breakdown
    FROM revenue-assurance-prod.ime_suspense.IME_Tableau_Summary
   UNION
    SELECT DISTINCT
      sap_exception AS breakdown
    FROM revenue-assurance-prod.control_a04q_rebill.alteryx_output
    WHERE
      sap_exception IN ('Exception, SAP data found but totals mismatch', 'SAP data not found', 'No SAP Exceptions')
   UNION),
          ...
          (
            SELECT
              ...
              COUNTIF(
                ...
                AND sap_exception IN ('Exception, SAP data found but totals mismatch', 'SAP data not found')
              ) AS control_count
            FROM
              scaffold
