...
CREATE OR REPLACE VIEW `revenue-assurance-prod.key_control_checklist.control_refresh_times` AS (
   WITH
      -- Refresh times for A02-Q, A17-M, and F01-M sourced from pulse.vw_project_tasks
      pulse_tasks_refresh AS (
         SELECT
            'A02-Q' AS control,
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.pulse.vw_project_tasks`
         UNION ALL
         SELECT
            'A17-M' AS control,
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
      ...
      a06m_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.control_a06m_leases.dim_lease_history`
      ),
      ...
      chv_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.billing_src.derived_attribute_array`
      ),
      ...
      f12m_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.control_f12m_btp_suspense.sim_tracker`
      ),
      -- Refresh times for FC01-Q and X01-B sourced from pulse_src.vessel
      pulse_vessel_refresh AS (
         SELECT
            'FC01-Q' AS control,
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.pulse_src.vessel`
         UNION ALL
         SELECT
            'X01-B' AS control,
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
      ...
      ime01w_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.ime_suspense.EPS_Suspense`
      ),
      ...
      ime02w_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.control_ime_sv.ime_summary`
      ),
      ...
      var1_refresh AS (
         SELECT
            ...
            TIMESTAMP_MILLIS(last_modified_time) AS last_refresh
         FROM
            `revenue-assurance-prod.control_var_01_leases.output_var_leases_alteryx_data`
      )
