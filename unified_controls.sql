...
CREATE OR REPLACE TABLE `revenue-assurance-prod.key_control_checklist.unified_controls` AS (
   WITH
      ...
      a02q_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_a02_fx_completeness.output_fx_completeness_snb_control_monthly_data` a02q ON csf.date = a02q.current_commissioning_confirmed_date
      ...
      a04q_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_a04q_rebill.alteryx_output` a04q ON csf.date = CAST(a04q.crc_created_on AS DATE)
      ...
      a06m_data AS (
         ...
         FROM
            ...
            ) a06m ON csf.date = CAST(a06m.contract_start_date AS DATE)
      ...
      a15q_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.key_control_checklist.a15q_extract` a15q ON csf.date = DATE(a15q.billing_task_completed_on)
         WHERE
            csf.control = 'A15-Q'
         GROUP BY
            csf.control,
      ...
      a17m_data AS (
         ...
         FROM
            ...
            ) a17m ON csf.date = CAST(a17m.billing_task_completed_on AS DATE)
      ...
      chv_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.key_control_checklist.chv_extract` chv ON csf.date = DATE(chv.charterer_start_date)
         WHERE
            csf.control = 'CH-V'
         GROUP BY
            csf.control,
      ...
      e05w_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_e05w.output_data` e05w ON csf.date = DATE(e05w.activation_date)
         WHERE
            csf.control = 'E05-W'
         GROUP BY
            csf.control,
      ...
      f01m_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_f01_m_pulse_projects_reconciliation.control_monthly_data` f01m ON csf.date = f01m.project_implementation_confirmed_date
      ...
      f12m_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_f12m_btp_suspense.tableau_summary` f12m ON csf.date = f12m.chargestartdate
      ...
      fc01q_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.key_control_checklist.fc01q_extract` fc01q ON csf.date = DATE(fc01q.commissioning_confirm_date)
         WHERE
            csf.control = 'FC01-Q'
         GROUP BY
            csf.control,
      ...
      gx4_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_gx4.output_control_outcomes` gx4 ON csf.date = DATE(gx4.control_date)
         WHERE
            csf.control = 'GX4-JX'
         GROUP BY
            csf.control,
      ...
      ime01w_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.ime_suspense.IME_Tableau_Summary` ime01w ON csf.date = ime01w.chargestartdate
      ...
      ime02w_data AS (
         ...
         FROM
            ...
            ) ime02w ON csf.date = ime02w.ime_ime_file_date
      ...
      var1_data AS (
         ...
         FROM
            ...
            ) var1 ON csf.date = CAST(var1.order_date AS DATE)
      ...
      x01b_data AS (
         ...
         FROM
            control_scaffold csf
            LEFT JOIN `revenue-assurance-prod.control_x01b_retail_fx_temprarary_stopped_vessels_review.control_output_data_temp_stop_vessels` x01b ON csf.date = x01b.stopped_confirmed_date
